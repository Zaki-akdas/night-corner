...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.product.updateMany({ where: { categoryId: id }, data: { categoryId: fallback.id } });
  await prisma.category.delete({ where: { id } });
  logActivity({ userId: admin.id, userName: admin.name ?? "Admin", action: "CATEGORY_DELETED", entity: "Category", entityId: id });
